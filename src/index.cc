...
Index_Record::Index_Record(const std::chrono::milliseconds &timestamp,
...
std::chrono::milliseconds Index_Record::timestamp() const { return _timestamp; }

std::string Index_Record::filename() const { return _filename; }

uint64_t Index_Record::offset() const { return _offset; }

uint64_t Index_Record::position() const { return _position; }
...
const std::vector<Index_Record> &Index::index() const { return _index; }
...
std::istream &operator>>(std::istream &in, Index_Record &i) {
  ...
  Index_Record r(std::chrono::milliseconds(timestamp), position, filename,
                 offset);
  i = r;
...
void Index::append(const Index_Record &r) {
  if (_index.empty()) {
    ...
    if ((r.position() > _index.back().position()) &&
...
Index::get_records(const std::string &chunk_name) const {
...
const std::vector<Index_Record>
Index::time_lookup(const std::chrono::milliseconds &start,
...
const std::vector<Index_Record> Index::position_lookup(const uint64_t &start,
                                                       const uint64_t &end) {
  assert(start >= 0);
  assert(start <= end);
  std::vector<Index_Record> records;
...
Index build_index(const Path &directory, const size_t &resolution) {
  std::vector<std::pair<std::chrono::milliseconds, Index_Record>>
      timestamp_records;
