...
std::vector<std::pair<std::chrono::milliseconds, std::string>> Archive::extract() const{
    const char *i = _raw.c_str() + _index;
    if(i == (_raw.c_str() + _raw.size())){
        throw End_Of_Archive();
    }
    ...
    while( i < end ){
        ...
        output.push_back(std::pair<std::chrono::milliseconds, std::string>(timestamp, std::string(i, blob_length)));
        i += blob_length;
    }
