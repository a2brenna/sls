...
void get_config(int argc, char *argv[]) {
  ...
  desc.add_options()("unix_domain_file",
                     po::value<std::string>(&unix_domain_file),
                     "Path to connect to server on")(
      "max_keys", po::value<size_t>(&CONFIG_MAX_KEYS),
      "Maximum number of test keys")("max_elements",
                                     po::value<size_t>(&CONFIG_MAX_ELEMENTS),
                                     "Maximum number of test elements per key");
...
int main(int argc, char *argv[]) {
  ...
  sls::global_server = std::shared_ptr<smpl::Remote_Address>(
      new smpl::Remote_UDS(unix_domain_file));
...

  {
    ...
    std::vector<std::string> test_data = {"1", "2", "3", "4", "5",
                                          "6", "7", "8", "9", "0"};
...

  {
    ...
    const unsigned long long num_keys =
        r() % CONFIG_MAX_KEYS; // up to 100 random keys
    ...
    std::cerr << "RANDOM_TEST_COMPLETE"
              << " nanos " << (end_time - start_time).count() << " elements "
