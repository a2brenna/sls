...
namespace sls{
    ...
    class Client{
        private:
            ...
            std::pair<sls::Response, std::vector<std::pair<std::chrono::milliseconds, std::string>>> _request(const sls::Request &request);
            std::vector<std::pair<std::chrono::milliseconds, std::string>> _interval(const std::string &key, const unsigned long long &start, const unsigned long long &end, const bool &is_time);
